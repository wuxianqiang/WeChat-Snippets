...
const wxJson = require('baidu-json/bd.json')
const u = undefined

let vueDirectives = [
  genAttribute('s-if', u, '在框架中，使用 `s-if` 来判断是否需要渲染该代码块'),
  genAttribute('s-else', 'v', '也可以用 `s-elif` 和 `s-else` 来添加一个 else 块'),
  genAttribute('s-elif', u, '也可以用 `s-elif` 和 `s-else` 来添加一个 else 块'),
  genAttribute('s-for', u, '在组件上使用 `s-for` 控制属性绑定一个数组，即可使用数组中各项的数据重复渲染该组件'),
  genAttribute('s-for-item', u, '使用 `s-for-item` 可以指定数组当前元素的变量名'),
  genAttribute('s-for-index', u, '使用 `s-for-index` 可以指定数组当前下标的变量名'),
  genAttribute(
    's-key',
    u,
...
export function getBaiduTagProvider(): IHTMLTagProvider {
  return {
    getId: () => 'wxml',
