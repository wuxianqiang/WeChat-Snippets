...
const wxJson = require('bytedance-json/bytedance.json')
const u = undefined

let vueDirectives = [
  genAttribute('tt:if', u, '在框架中，使用 `tt:if` 来判断是否需要渲染该代码块'),
  genAttribute('tt:else', 'v', '也可以用 `tt:elif` 和 `tt:else` 来添加一个 else 块'),
  genAttribute('tt:elif', u, '也可以用 `wx:elif` 和 `tt:else` 来添加一个 else 块'),
  genAttribute('tt:for', u, '在组件上使用 `tt:for` 控制属性绑定一个数组，即可使用数组中各项的数据重复渲染该组件'),
  genAttribute('tt:for-item', u, '使用 `tt:for-item` 可以指定数组当前元素的变量名'),
  genAttribute('tt:for-index', u, '使用 `tt:for-index` 可以指定数组当前下标的变量名'),
  genAttribute(
    'tt:key',
    u,
...
export function getByteTagProvider(): IHTMLTagProvider {
  return {
    getId: () => 'wxml',
