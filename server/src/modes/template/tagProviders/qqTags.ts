...
const wxJson = require('qq-json/qq.json');
const u = undefined;

const vueDirectives = [
  genAttribute('qq:if', u, '在框架中，使用 `qq:if` 来判断是否需要渲染该代码块'),
  genAttribute('qq:else', 'v', '也可以用 `qq:elif` 和 `qq:else` 来添加一个 else 块'),
  genAttribute('qq:elif', u, '也可以用 `qq:elif` 和 `qq:else` 来添加一个 else 块'),
  genAttribute('qq:for', u, '在组件上使用 `qq:for` 控制属性绑定一个数组，即可使用数组中各项的数据重复渲染该组件'),
  genAttribute('qq:for-item', u, '使用 `qq:for-item` 可以指定数组当前元素的变量名'),
  genAttribute('qq:for-index', u, '使用 `qq:for-index` 可以指定数组当前下标的变量名'),
  genAttribute(
    'qq:key',
    u,
...
export function getQqTagProvider(): IHTMLTagProvider {
  return {
    getId: () => 'wxml',
