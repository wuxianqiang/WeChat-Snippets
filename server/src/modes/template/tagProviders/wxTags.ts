...
const vueDirectives = [
  ...
  genAttribute('wx:else', 'v', '也可以用 `wx:elif` 和 `wx:else` 来添加一个 else 块'),
  ...
  genAttribute('wx:for-item', u, '使用 `wx:for-item` 可以指定数组当前元素的变量名'),
  genAttribute('wx:for-index', u, '使用 `wx:for-index` 可以指定数组当前下标的变量名'),
  genAttribute(
    'wx:key',
    'wx',
...
export interface IComponentSet {
  [componentName: string]: WXMLTagSpecification;
}
const wxTags: IComponentSet = {};

export function getComponentMarkdown(c: Component):string {
  ...
  if (c.relateApis) rows.push(...list('相关接口', c.relateApis.map((l:any) => link(l.name, l.link))))
  if (c.docLink) rows.push(link('官方文档', c.docLink))

  return rows.join('\n\n')
}

export function getComponentAttrMarkdown(a: ComponentAttr): string {
  ...
  if (a.subAttrs && !a.enum) rows.push(...list('可选值', a.subAttrs.map((s:any) => _formatAttrValue({ value: s.equal }))))
  if (a.extras) rows.push(...a.extras.filter((e:any) => e.key && e.value).map((e:any) => field(e.key, e.value)))

  return rows.join('\n\n')
}

export function getComponentAttrValueMarkdown(v: ComponentAttrValue): string {
...
function field(title: string, value: string): string {
  return `**${title}:** ${value}`
}

function _formatAttrValue(av: { value: string; desc?: string; since?: string }): string {
  let rows = [av.value]
...
(wxJson as Component[]).forEach((comp: Component) => {
  wxTags[comp.name] = new WXMLTagSpecification(
    { kind: 'markdown', value: getComponentMarkdown(comp) },
    comp.attrs
      ? comp.attrs.map((t: { name: string; desc?: string[] }) => genAttribute(t.name, u, t.desc ? getComponentAttrMarkdown(t) : ''))
      : []
  );
});
