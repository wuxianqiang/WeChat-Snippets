...
import {
  ...
  const wxJson = require('weixin-json/wx.json');
  const u = undefined;
  
  const vueDirectives = [
    genAttribute('ks:if', u, '在框架中，使用 `ks:if` 来判断是否需要渲染该代码块'),
    genAttribute('ks:else', 'v', '也可以用 `ks:elif` 和 `ks:else` 来添加一个 else 块'),
    genAttribute('ks:elif', u, '也可以用 `ks:elif` 和 `ks:else` 来添加一个 else 块'),
    genAttribute('ks:for', u, '在组件上使用 `ks:for` 控制属性绑定一个数组，即可使用数组中各项的数据重复渲染该组件'),
    genAttribute('ks:for-item', u, '使用 `ks:for-item` 可以指定数组当前元素的变量名'),
    genAttribute('ks:for-index', u, '使用 `ks:for-index` 可以指定数组当前下标的变量名'),
    genAttribute(
      'ks:key',
      u,
      '如果列表中项目的位置会动态改变或者有新的项目添加到列表中，并且希望列表中的项目保持自己的特征和状态（如 input 中的输入内容，switch 的选中状态），需要使用 `ks:key` 来指定列表中项目的唯一的标识符'
    ),
  ...
  weixinTags['template'] = new HTMLTagSpecification(
    { kind: 'markdown', value:  'KSML提供模板（template），可以在模板中定义代码片段，然后在不同的地方调用。' },
    [
  ...
  weixinTags['import'] = new HTMLTagSpecification(
    { kind: 'markdown', value:  'KSML 提供两种文件引用方式import和include。' },
    [
  ...
  export function getKuaishouTagProvider(): IHTMLTagProvider {
    return {
      getId: () => 'wxml',
