...
let vueDirectives = [
  ...
  genAttribute('jd:for', u, '在组件上使用 `jd:for` 控制属性绑定一个数组，即可使用数组中各项的数据重复渲染该组件'),
  genAttribute('jd:for-item', u, '使用 `jd:for-item` 可以指定数组当前元素的变量名'),
  genAttribute('jd:for-index', u, '使用 `jd:for-index` 可以指定数组当前下标的变量名'),
  genAttribute(
    'jd:key',
    u,
