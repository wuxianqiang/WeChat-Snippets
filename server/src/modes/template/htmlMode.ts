...
export class HTMLMode implements LanguageMode {
  ...
  doComplete(document: TextDocument, position: Position): any {
    const embedded = this.embeddedDocuments.refreshAndGet(document);
    ...
    return doComplete(embedded, position, this.vueDocuments.refreshAndGet(embedded), tagProviders, this.config.wxml);
  }
